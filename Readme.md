...
`esy-postgresql` is sample package, [`hello`](https://github.com/esy/test-hello-c), packaged for [`esy`](https://esy.sh/).
...
## How to use `esy-postgresql`?

`esy-postgresql` can be used from both NPM and directly from Github.

### From NPM 

`esy-postgresql` is deployed on NPM can be found
[here](https://www.npmjs.com/package/TODO).

You can simply run `esy add esy-postgresql` to install it, or specify it in
...
{
  "dependencies": {
    "esy-postgresql": "esy-packages/esy-postgresql"
  }
}
```
...
```diff
  "dependencies": {
+   "esy-postgresql": "esy-packages/esy-postgresql#<commit hash>"
  }
```
